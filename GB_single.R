...
if(!dir.exists('single_gb')){
  dir.create('single_gb')
}
setwd('single_gb')
...
dataframeforplot%>%ggplot(aes(x = name,y=value))+
  ...
  geom_text(x=1.5,y=5.5+adjust,label=str_c('Num.Driver.Mutated = ',length(patient_cgc)),color="#E69F00",size=5)+
  geom_text(x=1.5,y=5.2+adjust,label=str_c('Num.Driver.Unmutated = ',length(patient_ncgc)),color="#999999",size=5)+
  ...
  labs(title=str_c('AUC Distributions Between Mutation Groups on ',disease_list[disease]), y ="AUC", x = "Drug")
ggsave('cgc_AUC_box.pdf',width=8,height = 6)


# drug_response_breast_all$lgec50 <- 0-drug_response_breast_all$ec50%>%log()
...
dataframeforplot%>%ggplot(aes(x = name,y=value))+
  ...
  geom_text(data=dataframeforplot1,aes(y=4.6+adjust,label=str_c('p.w=',p_w),color=w.sig.threshold),size=3)+
  geom_text(data=dataframeforplot1,aes(y=4.4+adjust,label=str_c('p.m=',p_m),color=m.sig.threshold),size=3)+
  geom_text(x=1.5,y=5.5+adjust,label=str_c('Num.Driver.Mutated = ',length(patient_cgc)),color="#75FB53",size=5)+
  geom_text(x=1.5,y=5.2+adjust,label=str_c('Num.Driver.Unmutated = ',length(patient_ncgc)),color="#886fB9",size=5)+
  scale_fill_manual(values=c( "#75FB53", "#886fB9"))+
  scale_color_manual(values = c('#d11141','#f37735','#00aedb','#8c8c8c'),breaks=c('p<0.01','p<0.05','p<0.1','p>=0.1'))+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 0,vjust = 1,size = 10),axis.title.y = element_text(size = 10))+
  labs(title=str_c('EC50 Distributions Between Mutation Groups on ',disease_list[disease]), y ="log(EC50)", x = "Drug")
ggsave('cgc_EC50_box.pdf',width=8,height = 6)
...
for (j in 1:length(cgc_breast$Gene.Symbol)) {
  ...
  if(sum(flag1)>2&sum(flag2)>2){
    ...
    dataframeforplot%>%ggplot(aes(x = name,y=value))+
      geom_boxplot(aes(fill = tag),color = 'black', alpha=0.4)+
      stat_summary(fun = mean,geom = "point", shape=5, size=1.5,aes(group=tag), position = position_dodge(0.8))+
      geom_text(data=dataframeforplot1,aes(y=5+adjust,label=str_c('p.t=',p_t),color=sig.threshold),size=3)+
      geom_text(data=dataframeforplot1,aes(y=4.8+adjust,label=str_c('p.w=',p_w),color=w.sig.threshold),size=3)+
      geom_text(data=dataframeforplot1,aes(y=4.6+adjust,label=str_c('p.m=',p_m),color=m.sig.threshold),size=3)+
      geom_text(x=1.5,y=5.5+adjust,label=str_c('Num.',cgc_breast$Gene.Symbol[j],'.Mutated = ',length(patient_c)),color="#E69F00",size=5)+
      geom_text(x=1.5,y=5.3+adjust,label=str_c('Num.',cgc_breast$Gene.Symbol[j],'.Unmutated = ',length(patient_nc)),color="#999999",size=5)+
      scale_fill_manual(values=c( "#E69F00","#999999"))+
      scale_color_manual(values = c('#d11141','#f37735','#00aedb','#8c8c8c'),breaks=c('p<0.01','p<0.05','p<0.1','p>=0.1'))+
      theme_minimal()+
      theme(axis.text.x = element_text(angle = 0,vjust = 1,size = 10),axis.title.y = element_text(size = 10))+
    ...
    ggsave(output_file1,width=8,height = 6)
    ...
    dataframeforplot%>%ggplot(aes(x = name,y=value))+
      ...
      geom_text(x=1.5,y=5.5+adjust,label=str_c('Num.',cgc_breast$Gene.Symbol[j],'.Mutated = ',length(patient_c)),color="#75FB53",size=5)+
      geom_text(x=1.5,y=5.1+adjust,label=str_c('Num.',cgc_breast$Gene.Symbol[j],'.Unmutated = ',length(patient_nc)),color="#886fB9",size=5)+
      scale_fill_manual(values=c( "#75FB53","#886fB9"))+
      scale_color_manual(values = c('#d11141','#f37735','#00aedb','#8c8c8c'),breaks=c('p<0.01','p<0.05','p<0.1','p>=0.1'))+
      theme_minimal()+
      theme(axis.text.x = element_text(angle = 0,vjust = 1,size = 10),axis.title.y = element_text(size = 10))+
    ...
    ggsave(output_file2,width=8,height = 6)
  }
}
...
for (j in 1:length(drug_names)) {
  ...
  t_test_value <- t.test(ccle_cnv_breast_sub$auc ~ ccle_cnv_breast_sub$cnv_max)
  w_test_value <- wilcox.test(ccle_cnv_breast_sub$auc ~ ccle_cnv_breast_sub$cnv_max)
  
  tab <- as.data.frame(
    c(
    p.t = t_test_value$p.value%>%round(3),
    p.w = w_test_value$p.value%>%round(3),
    p.m = pvalue(median_test(ccle_cnv_breast_sub$auc ~ as.factor(ccle_cnv_breast_sub$cnv_max)))%>%round(3)
    )
  )
  
  output_file <- str_c('auc_',drug_names[j],'.pdf')
  p <- ccle_cnv_breast_sub%>%ggplot(aes(x=cnv_max,y=auc))+
    geom_boxplot(aes(fill = cnv_max))+
    stat_summary(fun = mean,geom = "point", shape=5, size=1.5,aes(group=cnv_max), position = position_dodge(0.8))+
    geom_text(data=dataframeforplot1,aes(y=Inf,label=str_c('n = ',num),vjust=3),size=6,color='#f37735')+
    theme_minimal()+
  ...
  ggdraw()+
    draw_plot(p,width = 1)+
    draw_plot(tableGrob(unname(tab)),x=0.8,y=0.25, width = 0.15)
  
  ggsave(output_file,width=5,height = 4)
  
  
  t_test_value <- t.test(ccle_cnv_breast_sub$lgec50 ~ ccle_cnv_breast_sub$cnv_max)
  w_test_value <- wilcox.test(ccle_cnv_breast_sub$lgec50 ~ ccle_cnv_breast_sub$cnv_max)
  
  tab <- as.data.frame(
    c(
      p.t = t_test_value$p.value%>%round(3),
      p.w = w_test_value$p.value%>%round(3),
      p.m = pvalue(median_test(ccle_cnv_breast_sub$lgec50 ~ as.factor(ccle_cnv_breast_sub$cnv_max)))%>%round(3)
    )
  )
  
  output_file <- str_c('ec50_',drug_names[j],'.pdf')
  p <- ccle_cnv_breast_sub%>%ggplot(aes(x=cnv_max,y=lgec50))+
    geom_boxplot(aes(fill = cnv_max))+
    stat_summary(fun = mean,geom = "point", shape=5, size=1.5,aes(group=cnv_max), position = position_dodge(0.8))+
    geom_text(data=dataframeforplot1,aes(y=Inf,label=str_c('n = ',num),vjust=4),size=6,color='#f37735')+
    theme_minimal()+
    scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest2"))+
